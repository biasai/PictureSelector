...
public class PictureSelectionModel {
    ...
    public void forResult(final int requestCode) {
        if (!DoubleUtils.isFastDoubleClick()) {//防快速点击
            ...
            //fixme 问题查到了，如果访问SD卡权限被拒绝；拍照完成时，就不会有照片，也不会选中。所以跳转图片选择器之前，一定要先获取SD卡权限。
            permissions.request(Manifest.permission.WRITE_EXTERNAL_STORAGE).subscribe(new Observer<Boolean>() {
                ...
                public void onNext(Boolean aBoolean) {
                    if (aBoolean) {
                        //PictureFileUtils.deleteCacheDirFile(selector.getActivity().getBaseContext());
                        //fixme 跳转图片选择器之前，必须获取SD卡读取权限。获取SD卡权限后，再跳转。
                        toPictureSelectorActivity(selector.getActivity(), requestCode);
                    } else {
                        Toast.makeText(selector.getActivity(),
                                selector.getActivity().getString(R.string.picture_jurisdiction), Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    //跳转图片选择器
    private void toPictureSelectorActivity(Activity activity, int requestCode) {
